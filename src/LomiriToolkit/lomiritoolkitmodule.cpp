...
#include "uclomirishapeoverlay_p.h"
#include "ucunits_p.h"
...
static const char engineProperty[] = "__lomiri_toolkit_plugin_data";

/******************************************************************************
 * LomiriToolkitModule
 */
LomiriToolkitModule* LomiriToolkitModule::create(QQmlEngine *engine, const QUrl &baseUrl)
{
    if (!engine) {
        return Q_NULLPTR;
    }
    LomiriToolkitModule *data = new LomiriToolkitModule(engine);
    data->m_baseUrl = QUrl(baseUrl.toString() + '/');
    engine->setProperty(engineProperty, QVariant::fromValue(data));
    return data;
}

void LomiriToolkitModule::registerWindowContextProperty()
{
    ...
    /* Ensure that setWindowContextProperty is called in the same thread (the
       ...
       https://bugs.launchpad.net/lomiri-ui-toolkit/+bug/1205556
    */
...
void LomiriToolkitModule::setWindowContextProperty(QWindow* focusWindow)
{
...
void LomiriToolkitModule::initializeContextProperties(QQmlEngine *engine)
{
    ...
    context->setContextProperty(QStringLiteral("i18n"), LomiriI18n::instance());
    ContextPropertyChangeListener *i18nChangeListener =
        new ContextPropertyChangeListener(context, QStringLiteral("i18n"));
    QObject::connect(LomiriI18n::instance(), SIGNAL(domainChanged()),
                     i18nChangeListener, SLOT(updateContextProperty()));
    QObject::connect(LomiriI18n::instance(), SIGNAL(languageChanged()),
                     i18nChangeListener, SLOT(updateContextProperty()));

    // We can't use 'Application' because it exists (undocumented)
    context->setContextProperty(QStringLiteral("LomiriApplication"), UCApplication::instance());
...
void LomiriToolkitModule::registerTypesToVersion(const char *uri, int major, int minor)
{
    ...
    qmlRegisterUncreatableType<LomiriI18n>(
    ...
    qmlRegisterSimpleSingletonType<UCLomiriAnimation>(uri, major, minor, "LomiriAnimation");
 ...
 * public API
 */
LomiriToolkitModule::LomiriToolkitModule(QObject *parent)
    : QObject(parent)
{
}

QUrl LomiriToolkitModule::baseUrl(QQmlEngine *engine)
{
    if (!engine) {
        return QUrl();
    }
    LomiriToolkitModule *data = engine->property(engineProperty).value<LomiriToolkitModule*>();
...
void LomiriToolkitModule::initializeModule(QQmlEngine *engine, const QUrl &pluginBaseUrl)
{
    ...
    const char *privateUri = "Lomiri.Components.Private";
 ...
 * Styles module
 */
void LomiriStylesModule::defineModule(const char *uri)
{
    // 1.2 styles
...
void LomiriStylesModule::undefineModule()
{
    // nothing yet
}
 ...
 * Labs module
 */
void LomiriLabsModule::initializeModule(QQmlEngine *engine, QQmlExtensionPlugin *plugin)
{
    Q_UNUSED(plugin);
...
void LomiriLabsModule::defineModule(const char *uri)
{
...
void LomiriLabsModule::undefineModule()
{
    // nothing yet
}
