...
Q_LOGGING_CATEGORY(ucAction, "lomiri.components.Action", QtMsgType::QtWarningMsg)
 ...
 *
 * \section2 Mnemonics
 * Since Lomiri.Components 1.3 Action supports mnemonics. Mnemonics are shortcuts
 ...
 * Since Lomiri.Components 1.3 Action supports the checkable/checked properties.
 * \qml
 * Button {
 *     action: Action {
 *         checkable: true
...
QString UCAction::text()
{
    ...
    if (!m_mnemonic.isEmpty()) {
        ...
        // https://bugs.launchpad.net/lomiri/+source/lomiri-ui-toolkit/+bug/1276808
 ...
 * In order to use an icon from the Lomiri theme, use the \l iconName property instead.
 */
