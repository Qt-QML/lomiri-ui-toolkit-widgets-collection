...
// The LomiriShape uses a simple and efficient method described by Chris Green in this paper from
...
ShapeShader::ShapeShader() :
    m_useDistanceFields(UCLomiriShape::useDistanceFields(QOpenGLContext::currentContext()))
{
    ...
    , m_geometry(attributeSet(), vertexCount, indexCount, indexType)
{
    ...
    qsgnode_set_description(this, QLatin1String("lomirishape"));
#endif
}
...
/*! \qmltype LomiriShape
    ...
    \endqml
*/
UCLomiriShape::UCLomiriShape(QQuickItem* parent)
    : QQuickItem(parent)
    , m_source(NULL)
...
// static
bool UCLomiriShape::useDistanceFields(const QOpenGLContext* openglContext)
{
...
bool UCLomiriShape::isVersionGreaterThanOrEqual(Version version)
{
    return static_cast<int>(m_version) >= static_cast<int>(version);
}

/*! \qmlproperty string LomiriShape::radius
...
*/
void UCLomiriShape::setRadius(const QString& radius)
{
...
/*! \qmlproperty enumeration LomiriShape::aspect

    This property defines the graphical style of the LomiriShape. The default value is \c
    LomiriShape.DropShadow.

    \note Setting this disables support for the deprecated \l borderSource property. Use the
    LomiriShapeOverlay item in order to provide the inset "pressed" aspect previously supported by
    that property.

    \list
    \li \b LomiriShape.Flat - no effects applied
    \li \b LomiriShape.Inset - inner shadow slightly moved downwards and bevelled bottom
    \li \b LomiriShape.DropShadow - outer non-blurred shadow slightly moved downwards
    \endlist
*/
void UCLomiriShape::setAspect(Aspect aspect)
{
...
/*! \qmlproperty real LomiriShape::relativeRadius
    \since Lomiri.Components 1.3

    This property defines a radius relative to the size of the LomiriShape. It is specified as a
...
*/
void UCLomiriShape::setRelativeRadius(qreal relativeRadius)
{
...
// Deprecation layer.
void UCLomiriShape::dropImageSupport()
{
...
/*! \qmlproperty variant LomiriShape::source
    \since Lomiri.Components 1.2

    This property sets the source provider of a texture rendered in the LomiriShape. Supported types
    ...
    Item {
        LomiriShape {
            source: Image { source: "lomiri.png" }
        }
    }
    \endqml
    ...
    the appropriate LomiriShape properties instead (\l sourceFillMode, \l sourceHorizontalAlignment
    and \l sourceVerticalAlignment).

    \note Setting this disables support for the deprecated \l image property.
*/
void UCLomiriShape::setSource(const QVariant& source)
{
    dropImageSupport();
...
/*! \qmlproperty real LomiriShape::sourceOpacity
    \since Lomiri.Components 1.2
    ...
    \note Setting this disables support for the deprecated \l image property.
*/
void UCLomiriShape::setSourceOpacity(qreal sourceOpacity)
{
    dropImageSupport();
...
/*! \qmlproperty enumeration LomiriShape::sourceFillMode
    \since Lomiri.Components 1.2

    This properties defines how the \l source texture fills the LomiriShape. The modes are the same
    ...
    \list
    \li \b LomiriShape.Stretch - the source is scaled non-uniformly to fit
    \li \b LomiriShape.PreserveAspectFit - the source is scaled uniformly to fit without cropping
    \li \b LomiriShape.PreserveAspectCrop - the source is scaled uniformly to fit with cropping
    \li \b LomiriShape.Pad - the source is not scaled
    \endlist
*/
void UCLomiriShape::setSourceFillMode(UCLomiriShape::FillMode sourceFillMode)
{
    dropImageSupport();
...
/*! \qmlproperty enumeration LomiriShape::sourceHorizontalWrapMode
    \qmlproperty enumeration LomiriShape::sourceVerticalWrapMode
    \since Lomiri.Components 1.2

    When the \l sourceFillMode is set to \c LomiriShape.Pad or \c LomiriShape.PreserveAspectFit or
    when the \l sourceScale and/or \l sourceTranslation properties are changed, the \l source
    texture might not cover the entire LomiriShape area. This property defines how the source
    texture wraps outside of its content area. The default value is \c LomiriShape.Transparent.

    Ensure \c LomiriShape.Repeat is used only when necessary (in case of an \c Image or \c
    AnimatedImage source) as it requires the underlying texture to be extracted from its atlas. That
    slows down the rendering speed since it prevents the QtQuick renderer to batch the LomiriShape
    with others.

    \note Some OpenGL ES 2 implementations do not support \c LomiriShape.Repeat with
    non-power-of-two sized source textures.
    \note Setting this disables support for the deprecated \l image property.

    \list
    \li \b LomiriShape.Transparent - the source is clamped to transparent
    \li \b LomiriShape.Repeat - the source is repeated indefinitely
    \endlist
*/
void UCLomiriShape::setSourceHorizontalWrapMode(UCLomiriShape::WrapMode sourceHorizontalWrapMode)
{
    dropImageSupport();
...
void UCLomiriShape::setSourceVerticalWrapMode(UCLomiriShape::WrapMode sourceVerticalWrapMode)
{
    dropImageSupport();
...
/*! \qmlproperty enumeration LomiriShape::sourceHorizontalAlignment
    \since Lomiri.Components 1.2

    This property defines how the \l source texture is horizontally aligned inside the LomiriShape
    area. The default value is \c LomiriShape.AlignLeft.

    \note Setting this disables support for the deprecated \l image property.

    \list
    \li \b LomiriShape.AlignLeft - the source is aligned to the left
    \li \b LomiriShape.AlignHCenter - the source is aligned to the horizontal center
    \li \b LomiriShape.AlignRight - the source is aligned to the right
    \endlist
*/
void UCLomiriShape::setSourceHorizontalAlignment(
    UCLomiriShape::HAlignment sourceHorizontalAlignment)
{
    dropImageSupport();
...
/*! \qmlproperty enumeration LomiriShape::sourceVerticalAlignment
    \since Lomiri.Components 1.2

    This property defines how the \l source texture is vertically aligned inside the LomiriShape
    area. The default value is \c LomiriShape.AlignTop.

    \note Setting this disables support for the deprecated \l image property.

    \list
    \li \b LomiriShape.AlignTop - the source is aligned to the top
    \li \b LomiriShape.AlignVCenter - the source is aligned to the vertical center
    \li \b LomiriShape.AlignBottom - the source is aligned to the bottom
    \endlist
*/
void UCLomiriShape::setSourceVerticalAlignment(UCLomiriShape::VAlignment sourceVerticalAlignment)
{
    dropImageSupport();
...
/*! \qmlproperty vector2d LomiriShape::sourceTranslation
    \since Lomiri.Components 1.2
    ...
    scrolling animations (using the \c LomiriShape.Repeat wrap mode), etc.

    \note Setting this disables support for the deprecated \l image property.
*/
void UCLomiriShape::setSourceTranslation(const QVector2D& sourceTranslation)
{
    dropImageSupport();
...
/*! \qmlproperty vector2d LomiriShape::sourceScale
    \since Lomiri.Components 1.2
    ...
    \note Setting this disables support for the deprecated \l image property.
*/
void UCLomiriShape::setSourceScale(const QVector2D& sourceScale)
{
    dropImageSupport();
...
// Deprecation layer.
void UCLomiriShape::dropColorSupport()
{
...
/*! \qmlproperty color LomiriShape::backgroundColor
    \since Lomiri.Components 1.2
...
*/
void UCLomiriShape::setBackgroundColor(const QColor& backgroundColor)
{
    dropColorSupport();
...
/*! \qmlproperty color LomiriShape::secondaryBackgroundColor
    \since Lomiri.Components 1.2

    This property defines the secondary background color. It is used when \l backgroundMode is set
    to \c LomiriShape.VerticalGradient. The default value is transparent black.

    \note Setting this disables support for the deprecated \l color and \l gradientColor properties.
*/
void UCLomiriShape::setSecondaryBackgroundColor(const QColor& secondaryBackgroundColor)
{
    dropColorSupport();
...
/*! \qmlproperty enumeration LomiriShape::backgroundMode
    \since Lomiri.Components 1.2

    This property defines the background rendering mode. The default value is \c
    LomiriShape.SolidColor.

    \note Setting this disables support for the deprecated \l color and \l gradientColor properties.

    \list
    \li \b LomiriShape.SolidColor - the color is \l backgroundColor
    \li \b LomiriShape.VerticalGradient - the color is a vertical gradient from \l backgroundColor
           to \l secondaryBackgroundColor.
    \endlist
*/
void UCLomiriShape::setBackgroundMode(BackgroundMode backgroundMode)
{
    dropColorSupport();
...
/*! \qmlproperty string LomiriShape::borderSource
    \deprecated
...
*/
void UCLomiriShape::setBorderSource(const QString& borderSource)
{
    ...
    //     shape.
    // qmlWarning(this) << "'borderSource' is deprecated. Use 'aspect' instead.";
...
/*! \qmlproperty color LomiriShape::color
    \deprecated

    This property defines the color used to fill the LomiriShape when there is no \l image set. If
...
*/
void UCLomiriShape::setColor(const QColor& color)
{
    Q_UNUSED(color);
...
/*! \qmlproperty color LomiriShape::gradientColor
    \deprecated

    This property defines the bottom color used for the vertical gradient filling the LomiriShape
    when there is no \l image set. As long as this property is not set, a single color (defined
    by \l color) is used to fill the LomiriShape.

    \note Use \l backgroundColor, \l secondaryBackgroundColor and \l backgroundMode instead.
*/
void UCLomiriShape::setGradientColor(const QColor& gradientColor)
{
    if (isVersionGreaterThanOrEqual(Version13))
...
/*! \qmlproperty Image LomiriShape::image
    \deprecated

    This property holds the \c Image or \c ShaderEffectSource rendered in the LomiriShape. In case
...
*/
void UCLomiriShape::setImage(const QVariant& image)
{
    if (isVersionGreaterThanOrEqual(Version13))
...
// there was a QML LomiriShape proxy. This is why we don't provide doc for it. We'll still have to
// maintain it for a while for compatibility reasons.
void UCLomiriShape::setStretched(bool stretched)
{
    if (!(m_flags & SourceApiSet)) {
...
void UCLomiriShape::setHorizontalAlignment(HAlignment horizontalAlignment)
{
    if (!(m_flags & SourceApiSet)) {
...
// Deprecation layer. Same comment as setStretched().
void UCLomiriShape::setVerticalAlignment(VAlignment verticalAlignment)
{
    if (!(m_flags & SourceApiSet)) {
...
// Deprecation layer.
void UCLomiriShape::updateFromImageProperties(QQuickItem* image)
{
    int alignment;

    // LomiriShape::stretched depends on Image::fillMode.
    ...
    // LomiriShape::horizontalAlignment depends on Image::horizontalAlignment.
    ...
    // LomiriShape::verticalAlignment depends on Image::verticalAlignment.
    ...
    setVerticalAlignment(static_cast<UCLomiriShape::VAlignment>(alignment));
}

// Deprecation layer.
void UCLomiriShape::connectToPropertyChange(
...
// Deprecation layer.
void UCLomiriShape::connectToImageProperties(QQuickItem* image)
{
...
// Deprecation layer.
void UCLomiriShape::_q_imagePropertiesChanged()
{
    updateFromImageProperties(qobject_cast<QQuickItem*>(sender()));
}

void UCLomiriShape::_q_gridUnitChanged()
{
...
void UCLomiriShape::_q_providerDestroyed(QObject* object)
{
    Q_UNUSED(object);
    m_sourceTextureProvider = NULL;
}

void UCLomiriShape::_q_textureChanged()
{
    m_flags |= DirtySourceTransform;
    update();
}

QString UCLomiriShape::propertyForVersion(quint16 version) const
{
...
void UCLomiriShape::componentComplete()
{
    QQuickItem::componentComplete();
    m_version = MINOR_VERSION(importVersion(this)) == 3 ? Version13 : Version12;
}

void UCLomiriShape::geometryChanged(const QRectF& newGeometry, const QRectF& oldGeometry)
{
...
void UCLomiriShape::updateSourceTransform(
...
QSGNode* UCLomiriShape::updatePaintNode(QSGNode* oldNode, UpdatePaintNodeData* data)
{
    Q_UNUSED(data);
...
QSGNode* UCLomiriShape::createSceneGraphNode() const
{
    return new ShapeNode;
}

void UCLomiriShape::updateMaterial(
...
void UCLomiriShape::updateGeometry(
